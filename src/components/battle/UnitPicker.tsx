  ...
  if (roster && roster.length === 9) {
    ...
    return (
      <>
        ...
        {dragGhost && (
          <div className="drag-ghost" style={{ left: dragGhost.x, top: dragGhost.y - DRAG_Y_OFFSET }}>{getUnitIcon(dragGhost.type) ? <img src={iconUrl(getUnitIcon(dragGhost.type)!)} alt="" className="w-8 h-8" style={{ imageRendering: 'pixelated' }} /> : dragGhost.emoji}</div>
        )}
